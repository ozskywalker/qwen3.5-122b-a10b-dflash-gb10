...
class DFlashProposer(SpecDecodeBaseProposer):
    ...
    def _get_dflash_block_table(
        ...
        if block_table is not None:
            # dflash-blocktable-unpad: stored draft table may carry padded
            # (num_reqs_padded) rows from the target FULL-cudagraph forward;
            # slice to the drafter's unpadded batch so FA2 varlen block_table
            # rows == cu_seqlens_q-1 (port of upstream PR #43982 gemma4 fix).
