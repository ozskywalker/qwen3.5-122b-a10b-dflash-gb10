...
class FlashInferMetadataBuilder(AttentionMetadataBuilder[FlashInferMetadata]):
    ...
    def __init__(
        ...
        # DFlash draft layers intentionally mix sliding_attention (window_left>0)
        # and full_attention (window_left=-1) within the same attention group.
        # The KV cache spec is already widened to FullAttentionSpec for all
        # DFlash layers, so normalising to full attention here is safe and
        # allows a single FlashInfer kernel to handle the whole group.
        if not self.global_hyperparameters.has_same_window_lefts:
            self.global_hyperparameters.window_left = -1
            self.global_hyperparameters.has_same_window_lefts = True
            self.global_hyperparameters.has_same_all_params = True
            self.window_left = -1
            ...
            if not prefill_use_trtllm:
                ...
                if use_dcp:
                    ...
                    assert (prefill_wrapper._context._window_left == self.window_left
                            or prefill_wrapper._context._window_left == -1)
                    ...
                    assert (prefill_wrapper._new_tokens._window_left == self.window_left
                            or prefill_wrapper._new_tokens._window_left == -1)
                    ...
                    # Allow mixed window_left in DFlash groups: either the wrapper
                    # is full-attention (wrapper=-1, ok for any layer), or the layer
                    # is full-attention (impl=-1, ok with any wrapper window since
                    # DFlash only proposes short token blocks anyway).
                    assert (prefill_wrapper._window_left == self.window_left
                            or prefill_wrapper._window_left == -1
                            or self.window_left == -1), (
                        f"window_left mismatch: wrapper={prefill_wrapper._window_left} "
                        f"impl={self.window_left}"
                    )
        ...
        if num_decode_tokens > 0:
            ...
            if not decode_use_trtllm:
                ...
                assert (decode_wrapper._window_left == self.window_left
                        or decode_wrapper._window_left == -1
                        or self.window_left == -1), (
                    f"window_left mismatch: wrapper={decode_wrapper._window_left} "
                    f"impl={self.window_left}"
                )
