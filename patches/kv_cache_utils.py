    ...
    for layer_name, layer_spec in kv_cache_spec.items():
        if layer_spec.page_size_bytes == max_page_size:
            ...
            if max_page_size % layer_page_size == 0:
                ...
                # Also scale page_size_padded if set: replacing block_size alone
                # doesn't change page_size_bytes when page_size_padded is pinned
                # (set by the mamba/attention alignment step).  Without scaling it,
                # the assertion fails because new_spec.page_size_bytes still
                # returns the old padded value instead of max_page_size.
                kwargs: dict = {"block_size": new_block_size}
                old_padded = getattr(layer_spec, "page_size_padded", None)
                if old_padded is not None:
                    kwargs["page_size_padded"] = old_padded * ratio
                new_spec = replace(layer_spec, **kwargs)
            elif (
